...
object EquipmentService
{
	...
	suspend fun equip(itemId: Int): Boolean = withServiceCall(bus, sessionIdProvider, "EquipmentService", "equip") { backend.equip(itemId) }

	suspend fun unequip(slot: Int): Boolean = withServiceCall(bus, sessionIdProvider, "EquipmentService", "unequip") { backend.unequip(slot) }
