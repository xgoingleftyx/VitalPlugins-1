...
object MagicService
{
	...
	suspend fun cast(spell: vital.api.ui.Spell): Boolean = withServiceCall(bus, sessionIdProvider, "MagicService", "cast") { backend.cast(spell) }

	suspend fun castOn(spell: vital.api.ui.Spell, target: Targetable): Boolean = withServiceCall(bus, sessionIdProvider, "MagicService", "castOn") { backend.castOn(spell, target) }
