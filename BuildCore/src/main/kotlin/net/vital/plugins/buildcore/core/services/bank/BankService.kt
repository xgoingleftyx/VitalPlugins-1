...
object BankService
{
	...
	suspend fun open(): Boolean = withServiceCall(bus, sessionIdProvider, "BankService", "open",
		restriction = OperationalRestriction.BANK_DISABLED, stakes = ActionStakes.MEDIUM) { backend.open() }

	suspend fun close(): Boolean = withServiceCall(bus, sessionIdProvider, "BankService", "close",
		restriction = OperationalRestriction.BANK_DISABLED, stakes = ActionStakes.MEDIUM) { backend.close() }

	suspend fun deposit(itemId: Int, amount: Int): Boolean = withServiceCall(bus, sessionIdProvider, "BankService", "deposit",
		restriction = OperationalRestriction.BANK_DISABLED, stakes = ActionStakes.MEDIUM) { backend.deposit(itemId, amount) }

	suspend fun depositAll(): Boolean = withServiceCall(bus, sessionIdProvider, "BankService", "depositAll",
		restriction = OperationalRestriction.BANK_DISABLED, stakes = ActionStakes.MEDIUM) { backend.depositAll() }

	suspend fun depositInventory(): Boolean = withServiceCall(bus, sessionIdProvider, "BankService", "depositInventory",
		restriction = OperationalRestriction.BANK_DISABLED, stakes = ActionStakes.MEDIUM) { backend.depositInventory() }

	suspend fun withdraw(itemId: Int, amount: Int): Boolean = withServiceCall(bus, sessionIdProvider, "BankService", "withdraw",
		restriction = OperationalRestriction.BANK_DISABLED, stakes = ActionStakes.MEDIUM) { backend.withdraw(itemId, amount) }
